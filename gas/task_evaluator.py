...
# class GenTaskEvaluator(TaskEvaluator):
#     data_field = None
#     def __init__(
#             self,
#             task_plan_df,
...
#             seed,
#         )
#         self.eval_metrics = eval_matrics
#         self.n_instance_per_task = n_instance_per_task
#         self.n_trials_per_instance = n_trials_per_instance
...
#     def _evaluate_task_plan(self, task_plan, model: GenModel) -> dict[str]:
#         results = []
#         for i in range(self.n_instance_per_task):
#             task = self._generate_task(task_plan, i)
#             for _ in range(self.n_trials_per_instance):
#                 output = model.gen(task['prompt'])
#                 results.append(self.eval_metrics(output, task))
#         # this is different from apply_aggregate_function, this is aggregated between different trials
#         aggregated_results = aggregate_results(results) 
#         assert all(isinstance(key, str) for key in aggregated_results.keys())
...
#         if self.eval_cache_path is None:
#             if isinstance(model, list):
#                 results = [self._evaluate_task_plan(task_plan, m) for m in model]
#             else:
#                 results = self._evaluate_task_plan(task_plan, model)
#         else:
#             with diskcache.Cache(self.eval_cache_path, size_limit=self.cache_size_limit) as cache:
#                 key = task_plan.copy()
#                 key['n_trials_per_instance'] = self.n_trials_per_instance
...
#                         key['model'] = m.model_name
#                         key_str = json.dumps(key, sort_keys=True)
#                         results_m = None if self.overwrite_eval_cache else cache.get(key_str, None)
#                         if results_m is None:
#                             results_m = self._evaluate_task_plan(task_plan, m)
#                             cache.set(key_str, results_m)
#                         results.append(results_m)
#                 else:
#                     key['model'] = model.model_name
#                     key_str = json.dumps(key, sort_keys=True)
#                     results = None if self.overwrite_eval_cache else cache.get(key_str, None)
#                     if results is None:
#                         results = self._evaluate_task_plan(task_plan, model)
#                         cache.set(key_str, results)
#         # the aggregate here means the aggregation between different task plan
#         res = apply_aggregate_function(results, aggregate_func)
#         return res
    
    
# class ImageGenTaskEvaluator(GenTaskEvaluator):
#     data_filed = 'image'
    
# class VideoGenTaskEvaluator(GenTaskEvaluator):
#     data_filed = 'video'
    
# class ThreeDGenTaskEvaluator(GenTaskEvaluator):
#     data_filed = 'threed'
